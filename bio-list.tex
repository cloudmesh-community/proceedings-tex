...
\begin{tiny}
\begin{verbatim}
Warning 1 in ../hid-sp18-401/technology/abstract-apache-ambari.tex line 4:
Command terminated with space. services like managing, monitoring and
provisioning to the hadoop clusters~\cite
^
\end{verbatim}
\end{tiny}
...
\begin{tiny}
\begin{verbatim}
Warning 18 in ../hid-sp18-401/technology/abstract-apache-impala.tex line 9: Use
either `` or '' as an alternative to `"'. data stored and run in Apache Hadoop
clusters. "It enables users to issue low-
^ Warning 18 in ../hid-sp18-401/technology/abstract-apache-impala.tex line 11:
Use either `` or '' as an alternative to `"'. data movement or
transformation"~\cite{hid-sp18-401-apache-Impala}.
^
\end{verbatim}
\end{tiny}
...
\begin{tiny}
\begin{verbatim}
Warning 18 in ../hid-sp18-401/technology/abstract-keras.tex line 3: Use either
`` or '' as an alternative to `"'. "Keras is a high-level neural networks API,
written in Python and capable of   ^ Warning 18 in ../hid-
sp18-401/technology/abstract-keras.tex line 4: Use either `` or '' as an
alternative to `"'. running on top of TensorFlow, CNTK or Theano"~\cite{hid-
sp18-401-keras}.                                               ^
\end{verbatim}
\end{tiny}
...
\begin{tiny}
\begin{verbatim}
Warning 18 in ../hid-sp18-401/technology/abstract-lightgbm.tex line 3: Use
either `` or '' as an alternative to `"'. "A fast, distributed, high performance
gradient boosting(GBDT, GBRT, GBM or   ^ Warning 36 in ../hid-
sp18-401/technology/abstract-lightgbm.tex line 3: You should put a space in
front of parenthesis. "A fast, distributed, high performance gradient
boosting(GBDT, GBRT, GBM or
^ Warning 18 in ../hid-sp18-401/technology/abstract-lightgbm.tex line 6: Use
either `` or '' as an alternative to `"'. of the DMTK project of
Microsoft"~\cite{hid- sp18-401-lightgbm}.                                   ^
Warning 36 in ../hid-sp18-401/technology/abstract-lightgbm.tex line 14: You
should put a space in front of parenthesis. boosting models on huge
datasets(that sometimes contain more than 500,000
^
\end{verbatim}
\end{tiny}
...
\color{blue}``\emph{XGBoost is an open-source software library which provides the gradient
boosting framework for C++, Java, Python,R, and Julia}''\color{black} ~\cite{hid-sp18-401
-XGBoost-wiki}.
...
\color{blue}``\emph{The name xgboost, though, actually refers to the engineering goal to push
the limit of computations resources for boosted tree algorithms. Which is the
reason why many people use xgboost}''\color{black}.  - says Tianqi Chen, creator of XGBoost
...
on github is \color{blue}``\emph{Scalable, Portable and Distributed Gradient Boosting (GBDT,
GBRT or GBM) Library, for Python, R, Java, Scala, C++ and more}''\color{black}~\cite{hid-
...
\begin{tiny}
\begin{verbatim}
Warning 39 in ../hid-sp18-401/technology/abstract-xgboost.tex line 4: Double
space found. boosting framework for C++, Java, Python,R, and Julia'' ~\cite{hid-
sp18-401                                                          ^
\end{verbatim}
\end{tiny}
...
Blockchains \color{blue}``\emph{are an example of a distributed computing system with
...
voting}''\color{black} ~\cite{hid-sp18-506-Blockchain}.
...
\href{https://github.com/cloudmesh-community/hid-sp18-506/blob/master//technology/abstract-blockchain.tex}{abstract-blockchain.tex}

 
chktex:
\begin{tiny}
\begin{verbatim}
Warning 39 in ../hid-sp18-506/technology/abstract-blockchain.tex line 26: Double
space found. voting'' ~\cite{hid-sp18-506-Blockchain}.           ^
\end{verbatim}
\end{tiny}
...
Ethereum is an open-source, public, distributed computing that \color{blue}``\emph{runs
...
interference}''\color{black} ~\cite{hid-sp18-506-EthereumOrg}.
...
\href{https://github.com/cloudmesh-community/hid-sp18-506/blob/master//technology/abstract-ethereum.tex}{abstract-ethereum.tex}

 
chktex:
\begin{tiny}
\begin{verbatim}
Warning 39 in ../hid-sp18-506/technology/abstract-ethereum.tex line 7: Double
space found. interference'' ~\cite{hid-sp18-506-EthereumOrg}.                 ^
\end{verbatim}
\end{tiny}
...
\color{blue}``\emph{An estimated 1.1 billion people, including many millions of
...
services}''\color{black}~\cite{hid-sp18-506-NoID}.
...
\href{https://github.com/cloudmesh-community/hid-sp18-506/blob/master//technology/abstract-id2020.tex}{abstract-id2020.tex}
...
\color{blue}``\emph{The Ripple network enables secure, instant and nearly free global
financial transactions of any size with no chargebacks.}''\color{black}
\cite{hid-sp18-506-RippleNetwork} \color{blue}``\emph{Ripple provides one frictionless
experience to send money globally using the power of
blockchain}''\color{black}~\cite{hid-sp18-506-GlobalPayment}.
...
\href{https://github.com/cloudmesh-community/hid-sp18-506/blob/master//technology/abstract-ripple.tex}{abstract-ripple.tex}
...
follow. \color{blue}``\emph{Emphasis is put on ease of use, performance, documentation,
...
be downloaded from http://scikit-learn.sourceforge.net}''\color{black}
...
\href{https://github.com/cloudmesh-community/hid-sp18-506/blob/master//technology/abstract-scikit-learn.tex}{abstract-scikit-learn.tex}
...
\begin{tiny}
\begin{verbatim}
Warning 24 in ../hid-sp18-511/technology/abstract-apttus.tex line 2: Delete this
space to maintain correct pagereferences. \index{CRM} \index{AI} \index{Quote-
to-cash}              ^ Warning 24 in ../hid-sp18-511/technology/abstract-
apttus.tex line 2: Delete this space to maintain correct pagereferences.
\index{CRM} \index{AI} \index{Quote-to-cash}                         ^ Warning
26 in ../hid-sp18-511/technology/abstract-apttus.tex line 12: You ought to
remove spaces in front of punctuation. process by using artificial intelligence
...
protocols}''\color{black}~\cite{hid-sp18-520-RabbitMQwiki}.
...
command line tool, rabbitmqadmin}''\color{black}~\cite{hid-sp18-520-RabbitMQmana}.
...
\color{blue}``\emph{ AWS DeepLens is the world's first video camera optimized to run
machine learning models and perform inference on the device. It comes
with 6 sample projects at launch, that you can deploy to your AWS
DeepLens in less than 10 minutes. You can run the sample projects as
is, connect them with other AWS services train a model in Amazon
Sagemaker and deploy it to AWS DeepLens, or extend the functionality
by triggering a lambda function when an action takes place. You can
even apply more advanced analytics on the cloud using Amazon Kinesis
Video Streams and Amazon Rekognition video.  AWS DeepLens provides the
building blocks for your machine learning
needs}''\color{black}\cite{hid-sp18-522-deeplens}.
...
\href{https://github.com/cloudmesh-community/hid-sp18-522/blob/master//technology/abstract-AWSDeepLens.tex}{abstract-AWSDeepLens.tex}
...
\color{blue}``\emph{AWS Fargate is a technology for Amazon ECS and EKS$*$ that allows you to run 
...
the infrastructure that runs them.}''\color{black}~\cite{hid-sp18-522-fargate}

\color{blue}``\emph{Amazon ECS and EKS have two modes: Fargate launch type and EC2 launch type.
...
government requirements.}''\color{black}~\cite{hid-sp18-522-fargate}
...
\href{https://github.com/cloudmesh-community/hid-sp18-522/blob/master//technology/abstract-AWSFargate.tex}{abstract-AWSFargate.tex}
...
servers which are pre-configured with storage where applications can
be deployed and managed.

\color{blue}``\emph{Amazon Lightshell provides developers compute, storage, and
networking capacity and capabilities to deploy and manage websites and
web applications in the cloud.  Several features include Virtual
Private Server, Powerful API, Highly available storage, Speedy and
secure networking, Access to AWS Simplified load
balancing}''\color{black}\cite{hid-sp18-522-lightsail}.

LightSail provides SSD based storage, a static IP at a very competing
price.  It has snapshot which has stored backup of each nodes that are
set-up.  Static IP address can be created which are fixed and assigned
to instances.  It provides two OS Amazon Linux and Ubuntu.
...
\href{https://github.com/cloudmesh-community/hid-sp18-522/blob/master//technology/abstract-AWSLightsail.tex}{abstract-AWSLightsail.tex}
...
\color{blue}``\emph{Go is an open source programming language that makes it easy to
...
community.}''\color{black}~\cite{hid-sp18-522-golanguage}.

\color{blue}``\emph{Go is expressive, concise, clean, and efficient. Its concurrency
...
language}''\color{black}~\cite{hid-sp18-522-golanguage}.
...
\href{https://github.com/cloudmesh-community/hid-sp18-522/blob/master//technology/abstract-TheGoLanguage.tex}{abstract-TheGoLanguage.tex}
...
\color{blue}``\emph{RIOT is a small operating system for networked, memory-constrained
...
Internet of Things}''\color{black}~\cite{hid-sp18-522-riot}.
...
\href{https://github.com/cloudmesh-community/hid-sp18-522/blob/master//technology/abstract-riotos-a.tex}{abstract-riotos-a.tex}
...
\end{IU}
\input{../hid-sp18-522/bio-swaroop-saurabh.tex}

\begin{IU}

hid-sp18-522

\url{https://github.com/cloudmesh-community/hid-sp18-522}

\url{https://github.com/cloudmesh-community/hid-sp18-522/issues}
...
\href{https://github.com/cloudmesh-community/hid-sp18-522/blob/master//technology/hid-sp18-522.bib}{../hid-sp18-522/technology/hid-sp18-522.bib}
